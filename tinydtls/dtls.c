...
dtls_write(struct dtls_context_t *ctx,
  ...
  if (!peer) { /* no ==> create one */
    int res;
    dtls_warn("dtls_write: no peer connection, creating one\n");
    ...
    if (peer->state != DTLS_STATE_CONNECTED) {
      dtls_warn("dtls_write: session exists but not connected\n");
      return 0;
    } else {
      dtls_warn("dtls_write: have connection - sending\n");
...
static void dtls_destroy_peer(dtls_context_t *ctx, dtls_peer_t *peer, int unlink)
{
  ...
  if (unlink) {
    ...
    dtls_dsrv_log_addr(DTLS_LOG_DEBUG, "sesh: removed peer", &peer->session);
  }
  dtls_free_peer(peer);
}
...
dtls_connect(dtls_context_t *ctx, const session_t *dst) {
  dtls_peer_t *peer;
  int res;
  dtls_warn("dtls_connect\n");
  peer = dtls_get_peer(ctx, dst);

  dtls_warn("get peer: %s\n", peer?"true":"false");
   ...
   * re-negotiation. */
  if (res > 0) {
    dtls_warn("invoke connection: %s\n", res?"true":"false");
    CALL(ctx, event, &peer->session, 0, DTLS_EVENT_CONNECT);

  } else if (res == 0) {
    dtls_warn("invoke renegotiation: %s\n", peer?"true":"false");
